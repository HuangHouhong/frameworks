...
public final class ViewRootImpl implements ViewParent,
    ...
    /**
     *  整个View绘制的开始，所有View的绘制都是从这里开始的
     */
    private void performTraversals() {
        // cache mView since it is used so much below...
        //这就是DecorView,即所有View的根布局
        ...
        // 这里WindowManager决定是否需要调用onMeasure方法两次  ？？没看懂
        ...
        if (mFirst) {
            ...
            //如果窗口类型是有状态栏了，那么DecorView的宽高需要除掉状态栏部分
            ...
            mAttachInfo.mUse32BitDrawingCache = true;   //需要32位绘图缓存，默认选择32位
            mAttachInfo.mHasWindowFocus = false;    //当前窗口没有焦点
            mAttachInfo.mWindowVisibility = viewVisibility;
            mAttachInfo.mRecomputeGlobalAttributes = false;     // 默认不触发全局更改
            viewVisibilityChanged = false;
            mLastConfiguration.setTo(host.getResources().getConfiguration());
            mLastSystemUiVisibility = mAttachInfo.mSystemUiVisibility;

            //如果没有指定布局方向，默认继承父布局方向。View.LAYOUT_DIRECTION_INHERIT实际上继承父布局的horizontal
...
//                        mAttachInfo.mHardwareRenderer.flushLayerUpdates();
            ...
            if (!mStopped) {
                ...
                if (focusChangedDueToTouchMode || mWidth != host.getMeasuredWidth()
                    ...
                    // 获取View宽高的测量规格
                    // mWidth，mHeight表示窗口的宽高，lp.width,lp.width表示DecorView的宽高
                    int childWidthMeasureSpec = getRootMeasureSpec(mWidth, lp.width);
                    int childHeightMeasureSpec = getRootMeasureSpec(mHeight, lp.width);
                    ...
                    // 如果measureAgain为true,那么再次调用performMeasure，这就是View#onMeasure方法为啥会被调用两次的原因
                    if (measureAgain) {
        ...
        //第一次绘制的时候，这个标记一定是didLayout一定是true
